            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-blue-800">₹{report.summary.totalAssets.toFixed(2)}</p>
                </div>
                <div className="bg-green-50 p-4 rounded-lg">
                  <p className="text-green-600 text-sm font-medium">Account Balance</p>
                  <p className="text-xl font-bold text-green-800">₹{report.summary.totalAccountBalance.toFixed(2)}</p>
                </div>
                <div className="bg-purple-50 p-4 rounded-lg">
                  <p className="text-purple-600 text-sm font-medium">FD Maturity Value</p>
                  <p className="text-xl font-bold text-purple-800">₹{report.summary.totalFDMaturityValue.toFixed(2)}</p>
                </div>
                <div className="bg-yellow-50 p-4 rounded-lg">
                  <p className="text-yellow-600 text-sm font-medium">Gold Current Value</p>
                  <p className="text-xl font-bold text-yellow-800">₹{report.summary.totalGoldCurrentValue.toFixed(2)}</p>
                </div>
                <div className="bg-indigo-50 p-4 rounded-lg">
                  <p className="text-indigo-600 text-sm font-medium">Net Cash Flow</p>
                  <p className={`text-xl font-bold ${report.summary.netCashFlow >= 0 ? 'text-green-800' : 'text-red-800'}`}>
                    ₹{report.summary.netCashFlow.toFixed(2)}
                  </p>
                </div>
                <div className="bg-pink-50 p-4 rounded-lg">
                  <p className="text-pink-600 text-sm font-medium">Gold P&L</p>
                  <p className={`text-xl font-bold ${(report.summary.totalGoldCurrentValue - report.summary.totalGoldPurchaseValue) >= 0 ? 'text-green-800' : 'text-red-800'}`}>
                ...
                <div className="bg-orange-50 p-4 rounded-lg">
                  <p className="text-orange-600 text-sm font-medium">Gold Weight</p>
                  <p className="text-xl font-bold text-orange-800">{report.summary.totalGoldWeight.toFixed(3)}g</p>
                </div>
              </div>
            </div>
                    ...
                    <div className="space-y-3 h-64 overflow-y-auto">
                      {report.accounts.map((account) => (
                        <div key={account.id} className="flex justify-between items-center p-3 bg-gray-50 rounded">
                          ...
                          <span className="text-green-600 font-bold">₹{account.balance.toFixed(2)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                        ...
                        <div key={fd.id} className="p-3 bg-gray-50 rounded">
                          <div className="flex justify-between items-start">
                            ...
                            <div className="text-right">
                              <p className="font-bold text-purple-600">₹{fd.amount.toFixed(2)}</p>
                    ...
                    {report.gold.map((gold) => (
                      <div key={gold.id} className="p-4 bg-yellow-50 rounded-lg border">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <span className="font-bold text-yellow-700">{gold.grams.toFixed(3)}g</span>
                        </div>
                        <div className="space-y-1">
                          ...
                          <div className="flex justify-between border-t pt-1">
                            ...
                            <span className={`font-bold ${((gold.grams * currentGoldRate) - gold.value) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  <div className="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {report.notifications.maturedFDs.length > 0 && (
                        <div>
                          <h4 className="font-medium text-red-600 mb-2">Matured FDs ({report.notifications.maturedFDs.length})</h4>
                          <div className="space-y-2">
                            {report.notifications.maturedFDs.map((fd, index) => (
                              <div key={index} className="p-3 bg-red-50 rounded border border-red-200">
                                ...
                                <p className="text-sm text-red-600">₹{fd.amount} - {fd.daysPastMaturity} days overdue</p>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {report.notifications.approachingMaturityFDs.length > 0 && (
                        <div>
                          <h4 className="font-medium text-orange-600 mb-2">Approaching Maturity ({report.notifications.approachingMaturityFDs.length})</h4>
                          <div className="space-y-2">
                            {report.notifications.approachingMaturityFDs.map((fd, index) => (
                              <div key={index} className="p-3 bg-orange-50 rounded border border-orange-200">
                                <p className="font-medium  text-gray-900">{fd.account.name}</p>
                                <p className="text-sm text-orange-600">₹{fd.amount} - {fd.daysUntilMaturity} days remaining</p>
                              </div>
                            ))}
                          </div>
                        </div>
